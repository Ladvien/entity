"""Simple pipeline execution example showing the Workflow pattern.

This script highlights the hybrid dev/prod pattern. The same workflow
runs locally using ``config/dev.yaml`` and can be deployed with
``config/prod.yaml`` without code changes.
"""
...
async def build_registries(workflow: dict[PipelineStage, List]) -> SystemRegistries:
    """Create registries and register workflow plugins."""
...
async def main() -> None:  # pragma: no cover - example code
    workflow = {
        PipelineStage.DO: [
            AutoGeneratedPlugin(
                hello_plugin, [PipelineStage.DO], "hello_plugin", PromptPlugin
            )
        ]
    }
