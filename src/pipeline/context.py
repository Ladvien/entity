...
class PluginContext:
    ...
    def add_conversation_entry(self, content: str, role: str, metadata: Optional[Dict[str, Any]] = None):
        self._state.conversation.append(
            ConversationEntry(content=content, role=role, timestamp=datetime.now(), metadata=metadata or {})
        )

    def get_conversation_history(self, last_n: Optional[int] = None) -> List[ConversationEntry]:
        return self._state.conversation[-last_n:] if last_n else list(self._state.conversation)
...
class SimpleContext(PluginContext):
    ...
    def think(self, thought: str):
        self.add_conversation_entry(thought, role="system", metadata={"type": "thought"})

    def use_tool(self, tool_name: str, **params):
        key = self.execute_tool(tool_name, params)
        # Wait for tool execution when results become available
        # Simple blocking on event loop
        async def _get():
            while key not in self._state.stage_results:
