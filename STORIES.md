...
## Story 1: Create Plugin Repository Infrastructure

### Description
Set up the infrastructure and tooling needed to support plugins as separate Git submodule repositories.

### Acceptance Criteria
- [ ] Repository templates are created for plugin submodules
- [ ] CI/CD pipelines are configured for submodule repositories
- [ ] Documentation exists for creating new plugin repositories
- [ ] Integration tests pass with submodule structure

### Subtasks

#### Task 1.1: Create Plugin Repository Template
**Instructions for Junior Engineer:**
1. Create a new repository called `entity-plugin-template`
2. Set up the following directory structure:
   - `src/` directory for plugin code
   - `tests/` directory for plugin tests
   - `docs/` directory for plugin documentation
   - `.github/workflows/` directory for CI/CD
3. Create a `pyproject.toml` file with:
   - Basic project metadata fields (name, version, description, authors)
   - Dependency on `entity-core` as a required package
   - Test dependencies (pytest, pytest-asyncio, pytest-cov)
   - Build system configuration using setuptools
4. Create a `README.md` template with sections for:
   - Plugin description
   - Installation instructions
   - Usage examples
   - Development setup
   - Contributing guidelines
5. Create a `.gitignore` file appropriate for Python projects
6. Add a `LICENSE` file (use the same license as entity-core)
7. Create a `Makefile` with common commands:
...
#### Task 1.2: Configure CI/CD for Plugin Repositories
**Instructions for Junior Engineer:**
1. Create GitHub Actions workflow file `.github/workflows/test.yml`
2. Configure the workflow to trigger on:
   - Push to main branch
   - Pull requests
   - Manual dispatch
3. Add job steps for:
   - Checking out code with submodules
   - Setting up Python (support versions 3.8-3.11)
   - Installing dependencies including entity-core
   - Running linting (ruff or flake8)
   - Running type checking (mypy)
   - Running tests with coverage
   - Uploading coverage reports to codecov
4. Create `.github/workflows/release.yml` for publishing:
   - Trigger on version tags (v*.*.*)
   - Build wheel and sdist packages
   - Publish to PyPI (using secrets for credentials)
5. Add branch protection rules requiring:
   - Passing tests before merge
   - At least one approval for PRs
   - Up-to-date branches before merge

#### Task 1.3: Document Submodule Setup Process
**Instructions for Junior Engineer:**
1. Create `docs/SUBMODULE_SETUP.md` in entity-core repository
2. Document the complete process for creating a new plugin repository:
   - How to fork the template repository
   - How to customize the template for specific plugins
   - How to set up repository secrets for CI/CD
   - How to configure PyPI publishing
3. Document how to add a plugin repository as a submodule:
   - Git commands for adding submodules
   - Where submodules should be placed in the directory structure
   - How to update submodule references
   - How to handle submodule dependencies in development
4. Create a decision tree diagram showing:
   - When to create a new plugin repository
   - When to add to existing plugin repository
   - When to keep in core
5. Add troubleshooting section covering:
   - Common submodule issues and solutions
   - How to resolve merge conflicts in submodules
   - How to rollback submodule updates
